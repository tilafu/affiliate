...
document.addEventListener('DOMContentLoaded', () => {
  ...
  */
  ...
  const groupsList = document.getElementById('groupsViewList'); // Fixed: use correct ID from HTML
  ...
  function renderFakeUsersList() {
    ...
    const usersHtml = users.map(user => `
      <div class="user-item ${state.users.selected?.id === user.id ? 'selected' : ''}" 
           data-user-id="${user.id}" onclick="selectFakeUser(${user.id})">
        <div class="user-avatar">
          ${user.avatar_url ? 
            `<img src="${escapeHtml(user.avatar_url)}" alt="${escapeHtml(user.display_name)}" onerror="this.style.display='none'">` : 
            `<div class="avatar-placeholder">${escapeHtml(user.display_name?.charAt(0) || user.username?.charAt(0) || '?')}</div>`
          }
        </div>
        <div class="user-info">
          ...
          <div class="user-meta">
            <span class="created-date">Created: ${formatDate(user.created_at)}</span>
          </div>
        </div>
        <div class="user-actions">
          <button class="btn btn-sm" onclick="editFakeUser(${user.id}); event.stopPropagation();" title="Edit User">
            <i class="fas fa-edit"></i>
          </button>
          <button class="btn btn-sm btn-danger" onclick="deleteFakeUser(${user.id}); event.stopPropagation();" title="Delete User">
